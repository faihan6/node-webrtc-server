...
/**
 * This class takes care of encrypting, decrypting and processing SRTP packets.
...
class SRTPContext{
    ...
    // TODO: Move rocSeqNoWithSSRCMap and srtcpIndexCount into ssrcInfo
    ssrcInfo = {}
     ...
     * For every call of `sendRTPToRemote`, the SRTPContext will call `onPacketReadyToSend` with the encrypted SRTP packet
     * 
     * 
     * @param {Function} Params.onRTPPacketReady - Callback that will be called when an RTP packet is ready to be used by the application
     * For every call of `handleSRTPFromRemote`, the SRTPContext will call `onRTPPacketReady` with the decrypted RTP packet
     * 
     */
    constructor({onPacketReadyToSend, onRTPPacketReady}){
        this.sendPacketToRemoteCallback = onPacketReadyToSend;
        this.rtpPacketReadyCallback = onRTPPacketReady;
    }
    ...
    handleSRTPFromRemote(packet, remote){
        ...
        if(payloadType == 96){
            const rtpPacket = this.decryptSRTP(packet, this.srtpParams.clientKeys);
            this.rtpPacketReadyCallback(rtpPacket);

        }
        else{

            const rtcpPacket = this.decryptSRTCP(packet, this.srtpParams.clientKeys);
            this.handleRTCP(rtcpPacket);
    ...
    handleRTCP(rtcpPacket){
        ...
        if(payloadType == 200){
            ...
            this.rtpPacketReadyCallback(rtcpPacket);
        }
    }
    // 10000000
//       000000
